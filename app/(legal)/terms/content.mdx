# Terms of Service

**Last updated:** May 4, 2026

By using Nyxie ("the Bot"), you agree to these terms.

## Description

Nyxie is a Discord bot that provides ranked leaderboards, 1v1 challenges, tournaments, and server management features.

## Eligibility

You must be at least 13 years old and comply with Discord's Terms of Service to use Nyxie.

## Acceptable Use

You agree NOT to:

- Exploit bugs or vulnerabilities in the bot
- Use the bot to harass, spam, or abuse other users
- Attempt to access data from servers you are not a member of
- Manipulate leaderboard rankings through unauthorized means
- Use automated tools to interact with the bot (self-bots, macros)
- Overload the bot's API or WebSocket endpoints

## Server Admin Responsibilities

Server administrators who add Nyxie are responsible for:

- Informing their members that the bot collects usage data (as described in our Privacy Policy)
- Configuring the bot appropriately for their community
- Managing leaderboard data and challenge results fairly

## Availability

Nyxie is provided "as is" with no guarantee of uptime or availability. We may:

- Take the bot offline for maintenance
- Update features or commands without notice
- Remove the bot from servers that violate these terms

## Limitation of Liability

We are not responsible for:

- Loss of data due to server issues or bugs
- Disputes between players over challenge or tournament results
- Actions taken by server administrators using admin commands
- Any damages arising from use of the bot

## Intellectual Property

Nyxie's source code, branding, and documentation are owned by Gowtham A. The bot is licensed under ISC.

## Termination

We reserve the right to:

- Block users or servers from using the bot
- Delete data associated with banned users or servers
- Discontinue the bot at any time

## Changes

We may update these terms. Continued use of the bot after changes constitutes acceptance.

## Contact

- GitHub: [github.com/gowtham-2oo5/nyxie-bot](https://github.com/gowtham-2oo5/nyxie-bot)
- Discord: Contact the server admin where Nyxie is installed
